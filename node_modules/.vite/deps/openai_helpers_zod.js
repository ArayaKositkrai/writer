...
var JSON_SCHEMA_ARRAY_SCHEMA_KEYWORDS = ["allOf", "anyOf", "items", "oneOf", "prefixItems"];
...
function stripUndefinedSchemaKeywords(schema, visited = /* @__PURE__ */ new Set()) {
  ...
  for (const keyword of Object.keys(schemaRecord)) {
    if (schemaRecord[keyword] === void 0) {
...
function normalizeRootRefAndAllOf(schema) {
  ...
  while (true) {
    ...
    if (schema.$ref === void 0 && !normalizedAnyOf) {
      return;
    }
  }
}
function inlineRootRefObject(schema) {
  ...
  for (const keyword of ["$defs", "definitions"]) {
    if (schema[keyword] !== void 0 && inlined[keyword] !== void 0) {
...
function normalizeRootAllOf(schema) {
  while (schema.allOf !== void 0) {
    if (Array.isArray(schema.allOf)) {
      for (const [index, branch2] of schema.allOf.entries()) {
        normalizeObjectAllOfBranches(branch2, ["allOf", String(index)], schema);
      }
    }
...
function normalizeRootAnyOf(schema) {
  ...
  const realBranches = anyOf.map((branch2, index) => ({ branch: branch2, index })).filter(({ branch: branch2 }) => branch2 !== false);
...
function rewriteLocalRefsIntoPromotedRootAnyOfBranch(root, branchIndex, definitionRenames) {
  const rewriteRef = (ref) => {
    ...
    if (parts === void 0 || parts[0] !== "anyOf" || parts[1] !== String(branchIndex)) {
      return ref;
    }
...
function isNullable(schema, root, seenRefs = /* @__PURE__ */ new Set()) {
  ...
  if (schema.type !== void 0 && schema.type !== "null" && !(Array.isArray(schema.type) && schema.type.includes("null"))) {
    return false;
  }
  if ("const" in schema && schema.const !== null) {
    return false;
  }
  if (schema.enum !== void 0 && (!Array.isArray(schema.enum) || !schema.enum.includes(null))) {
    return false;
  }
  ...
  if (schema.not !== void 0 || schema.if !== void 0 || schema.then !== void 0 || schema.else !== void 0) {
    return false;
  }
  return true;
}
function ensureStrictJsonSchema(jsonSchema, path, root) {
  ...
  if ((type === "array" || Array.isArray(type) && type.includes("array")) && currentItems === void 0) {
...
function normalizeArrayUnionWrapper(jsonSchema, root) {
  if (isRedundantUnionWrapperType(jsonSchema.type, "array") && jsonSchema.items === void 0 && Array.isArray(jsonSchema.anyOf) && jsonSchema.anyOf.every((branch) => isArrayOnlySchema(branch, root))) {
    delete jsonSchema.type;
  }
}
...
function assertNoNestedSchemaIds(schema) {
  const visit = (value, path) => {
    ...
    if (path.length > 0 && value.$id !== void 0) {
...
function rewriteLocalRefsIntoFilteredAnyOfBranches(root) {
  const rewriteRef = (ref) => {
    ...
    if (originalParts === void 0 || originalParts.length === 0) {
      return ref;
    }
...
function preserveAllOfRefTargets(root, rootOnly = false) {
  ...
  if (root.$defs !== void 0 && !isObject(root.$defs)) {
...
function preserveDiscardedAllOfPropertyRefTargets(root, discardedPaths) {
  ...
  const collectRefs = (value) => {
    ...
    if (typeof value.$ref === "string") {
      ...
      if (parts !== void 0 && discardedPaths.some((discardedPath) => parts.length >= discardedPath.length && discardedPath.every((part, index) => parts[index] === part))) {
        refsToPreserve.add(value.$ref);
      }
    }
    forEachJSONSchemaChild(value, [], (child) => {
...
function validateRefSchemas(schema, path, root) {
  ...
  if (ref !== void 0) {
    ...
    const resolved = resolveLocalRef(root, ref);
    if (resolved === void 0 || !isSchemaDefinition(resolved)) {
...
function resolveObjectAllOfBranch(schema, root, normalizing) {
  ...
  while (true) {
    ...
    if (resolved.allOf !== void 0 && !normalizing.has(resolved)) {
      ...
      if (resolved.$ref !== void 0 || resolved.allOf !== previousAllOf) {
        continue;
      }
    }
...
function mergeObjectAllOf(jsonSchema, path, root, normalizing = /* @__PURE__ */ new Set()) {
  ...
  if (!parentHasObjectShape && ["additionalProperties", "properties", "required", "type"].some((keyword) => keyword in jsonSchema)) {
    fail();
  }
  ...
  for (const [index, entry] of allOf.entries()) {
    ...
    if (hasObjectShapeWithoutAllOf(branch)) {
      branches.push({
        schema: branch,
        sourcePath: branch === entry ? [...path, "allOf", String(index)] : void 0
      });
  ...
  const mergedProperties = /* @__PURE__ */ Object.create(null);
    ...
    if (branch.properties !== void 0) {
      ...
      for (const [key, propertySchema] of Object.entries(branch.properties)) {
        propertyEntries.push({
          key,
          propertySchema,
          sourcePath: sourcePath === void 0 ? void 0 : [...sourcePath, "properties", key]
        });
      }
    }
  ...
  const discardedPropertyPaths = propertyEntries.filter(({ key, sourcePath }) => sourcePath !== void 0 && (collapsesToNull || allowedClosedProperties !== void 0 && !allowedClosedProperties.has(key))).map(({ sourcePath }) => sourcePath);
  preserveDiscardedAllOfPropertyRefTargets(root, discardedPropertyPaths);
  if (jsonSchema === root && root.$defs !== void 0) {
  ...
  for (const { key, propertySchema } of propertyEntries) {
    if (allowedClosedProperties !== void 0 && !allowedClosedProperties.has(key)) {
      continue;
    }
...
function zodV4ToJsonSchema(schema, options = {}) {
  ...
  const jsonSchema = toJSONSchema(schema, {
    ...
    override: ({ zodSchema, jsonSchema: jsonSchema2 }) => {
      const def = zodSchema._zod.def;
      if (def.type === "union" && "discriminator" in def && Array.isArray(jsonSchema2.oneOf)) {
        if (jsonSchema2.anyOf !== void 0) {
        ...
        jsonSchema2.anyOf = jsonSchema2.oneOf;
        delete jsonSchema2.oneOf;
      }
    }
  });
...
function zodResponseFormat(zodObject, name, props) {
  ...
  return makeParseableResponseFormat({
    ...
    json_schema: {
      ...
      name,
      strict: true,
      schema: isZodV4(zodSchema) ? zodV4ToJsonSchema(zodSchema, { schemaDefinitions }) : zodV3ToJsonSchema(zodSchema, { name, schemaDefinitions })
    }
...
function zodFunction(options) {
  ...
  return makeParseableTool({
    ...
    function: {
      name: options.name,
      parameters: isZodV4(zodSchema) ? zodV4ToJsonSchema(zodSchema) : zodV3ToJsonSchema(zodSchema, { name: options.name }),
      strict: true,
...
function zodRealtimeFunction(options) {
  ...
  return {
    type: "function",
    name: options.name,
    parameters: isZodV4(zodSchema) ? zodV4ToNonStrictJsonSchema(zodSchema) : zodV3ToNonStrictJsonSchema(zodSchema, { name: options.name }),
    ...options.description ? { description: options.description } : void 0
  };
}
export {
...
//# sourceMappingURL=openai_helpers_zod.js.map
